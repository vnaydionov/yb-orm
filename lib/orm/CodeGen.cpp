...
void CppCodeGenerator::write_h_file_header(ostream &out)
{
    ...
    out << "\tstatic const Yb::String get_table_name() { return _T(\""
        ...
        << "\t\t\tconst Yb::Expression &filter, const Yb::Expression &order_by = Yb::Expression(), int max_n = -1);\n"
        << "\t// constructors\n"
...
void CppCodeGenerator::write_cpp_file(ostream &out)
{
    ...
    out << class_name_ << " &" << class_name_ << "::operator=(const "
        ...
        << "\t\tconst Yb::Expression &filter, const Yb::Expression &order_by, int max_n)\n"
        << "{\n"
        << "\t" << class_name_ << "::ListPtr lst(new "
        << class_name_ << "::List());\n"
        << "\tYb::ObjectList rows;\n"
        << "\tsession.load_collection(rows, Yb::Expression(_T(\"" << table_name_ << "\")), filter, order_by, max_n);\n"
        << "\tif (rows.size()) {\n"
